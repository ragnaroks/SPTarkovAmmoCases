...
const newId: string = '6809141feeaeec021a3a4b00';
const assortId: string = idcalc(newId,0xff);

export default function addCustomAmmoCase_9x33r(logger: ILogger,customItemService: CustomItemService,tables: IDatabaseTables) {
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'custom .357 ammo case',
        shortName: '.357',
        description: 'custom .357 ammo case'
      },
      ch: {
        name: '客制 .357 弹药箱',
        shortName: '.357',
        description: '客制 .357 弹药箱'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovAmmoCases]：addCustomAmmoCase_9x33r，Error：' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovAmmoCases]：addCustomAmmoCase_9x33r，ID：' + createResult.itemId);
}
