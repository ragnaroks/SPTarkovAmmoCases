...
const newId: string = '68091e58beb8727e1dcd2200';
const assortId: string = idcalc(newId,0xff);

export default function addCustomAmmoCase_86x70(logger: ILogger,customItemService: CustomItemService,tables: IDatabaseTables) {
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'custom .338 Lapua Magnum ammo case',
        shortName: '.338 Lapua Magnum',
        description: 'custom .338 Lapua Magnum ammo case'
      },
      ch: {
        name: '客制 .338 Lapua Magnum 弹药箱',
        shortName: '.338 Lapua Magnum',
        description: '客制 .338 Lapua Magnum 弹药箱'
      }
    },
    overrideProperties: {
      ...
      Grids: [
        {
          ...
          _props: {
            ...
            filters: [
              {
                Filter: [ItemTpl.AMMO_86X70_TACX],
                ExcludedFilter: []
              }
            ],
            isSortingTable: false,
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovAmmoCases]：addCustomAmmoCase_86x70，Error：' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovAmmoCases]：addCustomAmmoCase_86x70，ID：' + createResult.itemId);
}
