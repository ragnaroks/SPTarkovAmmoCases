...
const newId: string = '6810884951c843f1acf0a400';
const assortId1: string = idcalc(newId,0xff);
const assortId2: string = idcalc(newId,0xfe);

export default function addCustomAmmoCase_PL1(logger: ILogger,customItemService: CustomItemService,itemHelper: ItemHelper,tables: IDatabaseTables) {
  const idArray = itemHelper
    ...
    .filter(function(value) {
      ...
      if(!template._props.PenetrationPower || template._props.PenetrationPower < 11) {return false;}
      if(!template._props.PenetrationPower || template._props.PenetrationPower > 20) {return false;}
      return true;
    });
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'custom PL1 ammo case',
        shortName: 'PL1',
        description: 'custom PL1 ammo case, can store ammos those penetration power between 11 and 20'
      },
      ch: {
        name: '客制 PL1 弹药箱',
        shortName: 'PL1',
        description: '客制 PL1 弹药箱，可存放穿透力 11 至 20 的弹药'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovAmmoCases]：addCustomAmmoCase_PL1，Error：' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovAmmoCases]：addCustomAmmoCase_PL1，ID：' + createResult.itemId);
}
