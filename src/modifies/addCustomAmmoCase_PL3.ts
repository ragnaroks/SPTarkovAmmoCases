...
const newId: string = '68108b4af1b8c46964449300';
const assortId1: string = idcalc(newId,0xff);
const assortId2: string = idcalc(newId,0xfe);

export default function addCustomAmmoCase_PL3(logger: ILogger,customItemService: CustomItemService,itemHelper: ItemHelper,tables: IDatabaseTables) {
  const idArray = itemHelper
    ...
    .filter(function(value) {
      ...
      if(!template._props.PenetrationPower || template._props.PenetrationPower < 31) {return false;}
      if(!template._props.PenetrationPower || template._props.PenetrationPower > 40) {return false;}
      return true;
    });
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'custom PL3 ammo case',
        shortName: 'PL3',
        description: 'custom PL3 ammo case, can store ammos those penetration power between 31 and 40'
      },
      ch: {
        name: '客制 PL3 弹药箱',
        shortName: 'PL3',
        description: '客制 PL3 弹药箱，可存放穿透力 31 至 40 的弹药'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovAmmoCases]：addCustomAmmoCase_PL3，Error：' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovAmmoCases]：addCustomAmmoCase_PL3，ID：' + createResult.itemId);
}
