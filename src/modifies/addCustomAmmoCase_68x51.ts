...
const newId: string = '68091b00402733a5848f8d00';
const assortId1: string = idcalc(newId,0xff);
const assortId2: string = idcalc(newId,0xfe);

export default function addCustomAmmoCase_68x51(logger: ILogger,customItemService: CustomItemService,tables: IDatabaseTables) {
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'custom 6.8x51 ammo case',
        shortName: '6.8x51',
        description: 'custom 6.8x51 ammo case'
      },
      ch: {
        name: '客制 6.8x51 弹药箱',
        shortName: '6.8x51',
        description: '客制 6.8x51 弹药箱'
      }
    },
    overrideProperties: {
      ...
      Grids: [
        {
          ...
          _props: {
            ...
            filters: [
              {
                Filter: [ItemTpl.AMMO_68X51_HYBRID],
                ExcludedFilter: []
              }
            ],
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovAmmoCases]：addCustomAmmoCase_68x51，Error：' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovAmmoCases]：addCustomAmmoCase_68x51，ID：' + createResult.itemId);
}
