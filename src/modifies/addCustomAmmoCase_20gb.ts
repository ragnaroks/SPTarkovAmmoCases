...
const newId: string = '68092504f821917d87510100';
const assortId: string = idcalc(newId,0xff);

export default function addCustomAmmoCase_20gb(logger: ILogger,customItemService: CustomItemService,tables: IDatabaseTables) {
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'custom 20/70 buck ammo case',
        shortName: '20/70 buck',
        description: 'custom 20/70 buck ammo case'
      },
      ch: {
        name: '客制 20/70 buck 弹药箱',
        shortName: '20/70 buck',
        description: '客制 20/70 buck 弹药箱'
      }
    },
    overrideProperties: {
      ...
      Grids: [
        {
          ...
          _props: {
            ...
            filters: [
              {
                Filter: [ItemTpl.AMMO_20G_FLECHETTE],
                ExcludedFilter: []
              }
            ],
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovAmmoCases]：addCustomAmmoCase_20gb，Error：' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovAmmoCases]：addCustomAmmoCase_20gb，ID：' + createResult.itemId);
}
