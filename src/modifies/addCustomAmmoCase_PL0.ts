...
const newId: string = '681058a713ee981aa38d8700';
const assortId1: string = idcalc(newId,0xff);
const assortId2: string = idcalc(newId,0xfe);

export default function addCustomAmmoCase_PL0(logger: ILogger,customItemService: CustomItemService,itemHelper: ItemHelper,tables: IDatabaseTables) {
  const idArray = itemHelper
    ...
    .filter(function(value) {
      ...
      if(!template._props.PenetrationPower || template._props.PenetrationPower < 1) {return false;}
      if(!template._props.PenetrationPower || template._props.PenetrationPower > 10) {return false;}
      return true;
    });
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'custom PL0 ammo case',
        shortName: 'PL0',
        description: 'custom PL0 ammo case, can store ammos those penetration power between 1 and 10'
      },
      ch: {
        name: '客制 PL0 弹药箱',
        shortName: 'PL0',
        description: '客制 PL0 弹药箱，可存放穿透力 1 至 10 的弹药'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovAmmoCases]：addCustomAmmoCase_PL0，Error：' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovAmmoCases]：addCustomAmmoCase_PL0，ID：' + createResult.itemId);
}
