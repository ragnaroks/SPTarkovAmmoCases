...
const newId: string = '680915e32e4f2e1ef0b88900';
const assortId: string = idcalc(newId,0xff);

export default function addCustomAmmoCase_127x33(logger: ILogger,customItemService: CustomItemService,tables: IDatabaseTables) {
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'custom .50 AE ammo case',
        shortName: '.50 AE',
        description: 'custom .50 AE ammo case'
      },
      ch: {
        name: '客制 .50 AE 弹药箱',
        shortName: '.50 AE',
        description: '客制 .50 AE 弹药箱'
      }
    },
    overrideProperties: {
      ...
      Grids: [
        {
          ...
          _props: {
            ...
            filters: [
              {
                Filter: [ItemTpl.AMMO_127X33_FMJ],
                ExcludedFilter: []
              }
            ],
          ...
          _props: {
            ...
            filters: [
              {
                Filter: [ItemTpl.AMMO_127X33_COPPER],
                ExcludedFilter: []
              }
            ],
            isSortingTable: false,
          ...
          _props: {
            ...
            filters: [
              {
                Filter: [ItemTpl.AMMO_127X33_HAWK_JSP],
                ExcludedFilter: []
              }
            ],
            isSortingTable: false,
          ...
          _props: {
            ...
            filters: [
              {
                Filter: [ItemTpl.AMMO_127X33_JHP],
                ExcludedFilter: []
              }
            ],
            isSortingTable: false,
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovAmmoCases]：addCustomAmmoCase_127x33，Error：' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovAmmoCases]：addCustomAmmoCase_127x33，ID：' + createResult.itemId);
}
