...
const newId: string = '68091fff09a0386549a3f500';
const assortId: string = idcalc(newId,0xff);

export default function addCustomAmmoCase_127x55(logger: ILogger,customItemService: CustomItemService,tables: IDatabaseTables) {
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'custom 12.7x55 ammo case',
        shortName: '12.7x55',
        description: 'custom 12.7x55 ammo case'
      },
      ch: {
        name: '客制 12.7x55 弹药箱',
        shortName: '12.7x55',
        description: '客制 12.7x55 弹药箱'
      }
    },
    overrideProperties: {
      ...
      Grids: [
        {
          ...
          _props: {
            ...
            filters: [
              {
                Filter: [ItemTpl.AMMO_127X55_PS12B],
                ExcludedFilter: []
              }
            ],
          ...
          _props: {
            ...
            filters: [
              {
                Filter: [ItemTpl.AMMO_127X55_PS12],
                ExcludedFilter: []
              }
            ],
            isSortingTable: false,
          ...
          _props: {
            ...
            filters: [
              {
                Filter: [ItemTpl.AMMO_127X55_PS12A],
                ExcludedFilter: []
              }
            ],
            isSortingTable: false,
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovAmmoCases]：addCustomAmmoCase_127x55，Error：' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovAmmoCases]：addCustomAmmoCase_127x55，ID：' + createResult.itemId);
}
