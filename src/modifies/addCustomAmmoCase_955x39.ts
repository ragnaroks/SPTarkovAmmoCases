...
const newId: string = '68091f7282a7c5319d1a1100';
const assortId1: string = idcalc(newId,0xff);
const assortId2: string = idcalc(newId,0xfe);

export default function addCustomAmmoCase_955x39(logger: ILogger,customItemService: CustomItemService,tables: IDatabaseTables) {
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'custom .366 TKM ammo case',
        shortName: '.366 TKM',
        description: 'custom .366 TKM ammo case'
      },
      ch: {
        name: '客制 .366 TKM 弹药箱',
        shortName: '.366 TKM',
        description: '客制 .366 TKM 弹药箱'
      }
    },
    overrideProperties: {
      ...
      Grids: [
        {
          ...
          _props: {
            ...
            filters: [
              {
                Filter: [ItemTpl.AMMO_366TKM_APM],
                ExcludedFilter: []
              }
            ],
          ...
          _props: {
            ...
            filters: [
              {
                Filter: [ItemTpl.AMMO_366TKM_EKO],
                ExcludedFilter: []
              }
            ],
            isSortingTable: false,
          ...
          _props: {
            ...
            filters: [
              {
                Filter: [ItemTpl.AMMO_366TKM_FMJ],
                ExcludedFilter: []
              }
            ],
            isSortingTable: false,
          ...
          _props: {
            ...
            filters: [
              {
                Filter: [ItemTpl.AMMO_366TKM_GEKSA],
                ExcludedFilter: []
              }
            ],
            isSortingTable: false,
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovAmmoCases]：addCustomAmmoCase_955x39，Error：' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovAmmoCases]：addCustomAmmoCase_955x39，ID：' + createResult.itemId);
}
