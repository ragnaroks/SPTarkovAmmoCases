...
const newId: string = '68108b9ecc89ecfe1d878800';
const assortId1: string = idcalc(newId,0xff);
const assortId2: string = idcalc(newId,0xfe);

export default function addCustomAmmoCase_PL5(logger: ILogger,customItemService: CustomItemService,itemHelper: ItemHelper,tables: IDatabaseTables) {
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'custom PL5 ammo case',
        shortName: 'PL5',
        description: 'custom PL5 ammo case, can store ammos those penetration power between 51 and 60'
      },
      ch: {
        name: '客制 PL5 弹药箱',
        shortName: 'PL5',
        description: '客制 PL5 弹药箱，可存放穿透力 51 至 60 的弹药'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovAmmoCases]：addCustomAmmoCase_PL5，Error：' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovAmmoCases]：addCustomAmmoCase_PL5，ID：' + createResult.itemId);
}
