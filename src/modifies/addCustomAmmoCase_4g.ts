...
const newId: string = '6809266fc1cb707e5ee48100';
const assortId1: string = idcalc(newId,0xff);
const assortId2: string = idcalc(newId,0xfe);

export default function addCustomAmmoCase_4g(logger: ILogger,customItemService: CustomItemService,tables: IDatabaseTables) {
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'custom 23x75R ammo case',
        shortName: '23x75R',
        description: 'custom 23x75R ammo case'
      },
      ch: {
        name: '客制 23x75R 弹药箱',
        shortName: '23x75R',
        description: '客制 23x75R 弹药箱'
      }
    },
    overrideProperties: {
      ...
      Grids: [
        {
          ...
          _props: {
            ...
            filters: [
              {
                Filter: [ItemTpl.AMMO_23X75_BARRIKADA],
                ExcludedFilter: []
              }
            ],
          ...
          _props: {
            ...
            filters: [
              {
                Filter: [ItemTpl.AMMO_23X75_SHRAP10],
                ExcludedFilter: []
              }
            ],
            isSortingTable: false,
          ...
          _props: {
            ...
            filters: [
              {
                Filter: [ItemTpl.AMMO_23X75_SHRAP25],
                ExcludedFilter: []
              }
            ],
            isSortingTable: false,
          ...
          _props: {
            ...
            filters: [
              {
                Filter: [ItemTpl.AMMO_23X75_ZVEZDA],
                ExcludedFilter: []
              }
            ],
            isSortingTable: false,
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovAmmoCases]：addCustomAmmoCase_4g，Error：' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovAmmoCases]：addCustomAmmoCase_4g，ID：' + createResult.itemId);
}
