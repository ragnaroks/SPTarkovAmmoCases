...
const newId: string = '68108beaedd53f6542dcf500';
const assortId1: string = idcalc(newId,0xff);
const assortId2: string = idcalc(newId,0xfe);

export default function addCustomAmmoCase_PL6(logger: ILogger,customItemService: CustomItemService,itemHelper: ItemHelper,tables: IDatabaseTables) {
  const idArray = itemHelper
    ...
    .filter(function(value) {
      ...
      if(!template._props.PenetrationPower || template._props.PenetrationPower < 61) {return false;}
      //if(!template._props.PenetrationPower || template._props.PenetrationPower > 70) {return false;}
      return true;
    });
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'custom PL6 ammo case',
        shortName: 'PL6',
        description: 'custom PL6 ammo case, can store ammos those penetration power above 61'
      },
      ch: {
        name: '客制 PL6 弹药箱',
        shortName: 'PL6',
        description: '客制 PL6 弹药箱，可存放穿透力 61 或更高的弹药'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovAmmoCases]：addCustomAmmoCase_PL6，Error：' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovAmmoCases]：addCustomAmmoCase_PL6，ID：' + createResult.itemId);
}
