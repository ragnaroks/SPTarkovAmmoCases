...
const newId: string = '68108b72c051b4b9f9ea9800';
const assortId1: string = idcalc(newId,0xff);
const assortId2: string = idcalc(newId,0xfe);

export default function addCustomAmmoCase_PL4(logger: ILogger,customItemService: CustomItemService,itemHelper: ItemHelper,tables: IDatabaseTables) {
  const idArray = itemHelper
    ...
    .filter(function(value) {
      ...
      if(!template._props.PenetrationPower || template._props.PenetrationPower < 41) {return false;}
      if(!template._props.PenetrationPower || template._props.PenetrationPower > 50) {return false;}
      return true;
    });
  ...
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'custom PL4 ammo case',
        shortName: 'PL4',
        description: 'custom PL4 ammo case, can store ammos those penetration power between 41 and 50'
      },
      ch: {
        name: '客制 PL4 弹药箱',
        shortName: 'PL4',
        description: '客制 PL4 弹药箱，可存放穿透力 41 至 50 的弹药'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovAmmoCases]：addCustomAmmoCase_PL4，Error：' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovAmmoCases]：addCustomAmmoCase_PL4，ID：' + createResult.itemId);
}
