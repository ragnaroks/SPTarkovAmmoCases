...
const newId: string = '68090b87e3db563375934000';
const assortId: string = idcalc(newId,0xff);

export default function addCustomAmmoCase_9x21(logger: ILogger,customItemService: CustomItemService,tables: IDatabaseTables) {
  const newItem: NewItemFromCloneDetails = {
    ...
    locales: {
      en: {
        name: 'custom 9x21 ammo case',
        shortName: '9x21',
        description: 'custom 9x21 ammo case'
      },
      ch: {
        name: '客制 9x21 弹药箱',
        shortName: '9x21',
        description: '客制 9x21 弹药箱'
      }
    },
    overrideProperties: {
  ...
  if(!createResult.success) {
    logger.error('[SPTarkovAmmoCases]：addCustomAmmoCase_9x21，Error：' + createResult.errors.join('、'));
    return;
  }
  ...
  logger.success('[SPTarkovAmmoCases]：addCustomAmmoCase_9x21，ID：' + createResult.itemId);
}
