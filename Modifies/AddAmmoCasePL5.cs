...
public class AddAmmoCasePL5 : IOnLoad {
    private ISptLogger<AddAmmoCasePL5> Logger { get; }
    ...
    private MongoId BaseId { get; } = new("68108b9ecc89ecfe1d878800");
    private MongoId NewId { get; } = new("68108b9ecc89ecfe1d878801");
    private MongoId RotateId { get; set; } = new("68108b9ecc89ecfe1d878820");

#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCasePL5 (ISptLogger<AddAmmoCasePL5> logger, DatabaseService databaseService, CustomItemService customItemService, ItemHelper itemHelper) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        foreach (MongoId id in ammoTpls) {
            ...
            if (template.Properties.PenetrationPower is null or < 50 or > 59) { continue; }
            _ = itemTpls.Add(id);
        }
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "PL5 ammo case",ShortName = "PL5",Description = "skydust™ PL5 ammo case, can store ammos those penetration power between 50 and 59"}},
                {"ch",new(){Name = "PL5 弹药箱",ShortName = "PL5",Description = "skydust™ PL5 弹药箱，可存放穿透力 50 至 59 的弹药"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCasePL5.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCasePL5.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCasePL5.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
