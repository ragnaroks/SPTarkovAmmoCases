...
public class AddAmmoCasePLMax : IOnLoad {
    private ISptLogger<AddAmmoCasePLMax> Logger { get; }
    ...
    private MongoId BaseId { get; } = new("68108beaedd53f6542dcf900");
    private MongoId NewId { get; } = new("68108beaedd53f6542dcf901");
    private MongoId RotateId { get; set; } = new("68108beaedd53f6542dcf920");

#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCasePLMax (ISptLogger<AddAmmoCasePLMax> logger, DatabaseService databaseService, CustomItemService customItemService, ItemHelper itemHelper) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        foreach (MongoId id in ammoTpls) {
            ...
            if (template.Properties.PenetrationPower is null or < 70) { continue; }
            _ = itemTpls.Add(id);
        }
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "PLMax ammo case",ShortName = "PLMax",Description = "skydust™ PLMax ammo case, can store ammos those penetration power greater than or equal 70"}},
                {"ch",new(){Name = "PLMax 弹药箱",ShortName = "PLMax",Description = "skydust™ PLMax 弹药箱，可存放穿透力 70 或更高的弹药"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCasePLMax.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCasePLMax.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCasePLMax.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
