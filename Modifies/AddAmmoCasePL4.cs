...
public class AddAmmoCasePL4 : IOnLoad {
    private ISptLogger<AddAmmoCasePL4> Logger { get; }
    ...
    private MongoId BaseId { get; } = new("68108b72c051b4b9f9ea9800");
    private MongoId NewId { get; } = new("68108b72c051b4b9f9ea9801");
    private MongoId RotateId { get; set; } = new("68108b72c051b4b9f9ea9820");

#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCasePL4 (ISptLogger<AddAmmoCasePL4> logger, DatabaseService databaseService, CustomItemService customItemService, ItemHelper itemHelper) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        foreach (MongoId id in ammoTpls) {
            ...
            if (template.Properties.PenetrationPower is null or < 40 or > 49) { continue; }
            _ = itemTpls.Add(id);
        }
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "PL4 ammo case",ShortName = "PL4",Description = "skydust™ PL4 ammo case, can store ammos those penetration power between 40 and 49"}},
                {"ch",new(){Name = "PL4 弹药箱",ShortName = "PL4",Description = "skydust™ PL4 弹药箱，可存放穿透力 40 至 49 的弹药"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCasePL4.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCasePL4.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCasePL4.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
