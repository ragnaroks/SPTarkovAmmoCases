...
public class AddAmmoCase762x39 : IOnLoad {
    private ISptLogger<AddAmmoCase762x39> Logger { get; }
    private DatabaseService DatabaseService { get; }
    private CustomItemService CustomItemService { get; }
    private Double HandbookPrice { get; } = 50_0000D;
    private MongoId BaseId { get; } = new("68091c3a074a441c95ac5a00");
    private MongoId NewId { get; } = new("68091c3a074a441c95ac5a01");
    private MongoId RotateId { get; set; } = new("68091c3a074a441c95ac5a20");
...
#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCase762x39 (ISptLogger<AddAmmoCase762x39> logger, DatabaseService databaseService, CustomItemService customItemService) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "7.62x39 ammo case",ShortName = "7.62x39",Description = "skydust™ 7.62x39 ammo case"}},
                {"ch",new(){Name = "7.62x39 弹药箱",ShortName = "7.62x39",Description = "skydust™ 7.62x39 弹药箱"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase762x39.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase762x39.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCase762x39.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
