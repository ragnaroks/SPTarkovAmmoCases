...
public class AddAmmoCase762x35BLK : IOnLoad {
    private ISptLogger<AddAmmoCase762x35BLK> Logger { get; }
    private DatabaseService DatabaseService { get; }
    private CustomItemService CustomItemService { get; }
    private Double HandbookPrice { get; } = 50_0000D;
    private MongoId BaseId { get; } = new("68091bb49e2f08e6caff0000");
    private MongoId NewId { get; } = new("68091bb49e2f08e6caff0001");
    private MongoId RotateId { get; set; } = new("68091bb49e2f08e6caff0020");
...
#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCase762x35BLK (ISptLogger<AddAmmoCase762x35BLK> logger, DatabaseService databaseService, CustomItemService customItemService) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = ".300BLK ammo case",ShortName = ".300BLK",Description = "skydust™ .300BLK ammo case"}},
                {"ch",new(){Name = ".300BLK 弹药箱",ShortName = ".300BLK",Description = "skydust™ .300BLK 弹药箱"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase762x35BLK.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase762x35BLK.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCase762x35BLK.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
