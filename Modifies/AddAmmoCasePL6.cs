...
public class AddAmmoCasePL6 : IOnLoad {
    private ISptLogger<AddAmmoCasePL6> Logger { get; }
    ...
    private MongoId BaseId { get; } = new("68108beaedd53f6542dcf500");
    private MongoId NewId { get; } = new("68108beaedd53f6542dcf501");
    private MongoId RotateId { get; set; } = new("68108beaedd53f6542dcf520");

#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCasePL6 (ISptLogger<AddAmmoCasePL6> logger, DatabaseService databaseService, CustomItemService customItemService, ItemHelper itemHelper) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        foreach (MongoId id in ammoTpls) {
            ...
            if (template.Properties.PenetrationPower is null or < 60 or > 69) { continue; }
            _ = itemTpls.Add(id);
        }
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "PL6 ammo case",ShortName = "PL6",Description = "skydust™ PL6 ammo case, can store ammos those penetration power between 60 and 69"}},
                {"ch",new(){Name = "PL6 弹药箱",ShortName = "PL6",Description = "skydust™ PL6 弹药箱，可存放穿透力 60 至 69 的弹药"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCasePL6.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCasePL6.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCasePL6.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
