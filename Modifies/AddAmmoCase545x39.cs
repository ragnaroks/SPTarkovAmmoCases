...
public class AddAmmoCase545x39 : IOnLoad {
    private ISptLogger<AddAmmoCase545x39> Logger { get; }
    private DatabaseService DatabaseService { get; }
    private CustomItemService CustomItemService { get; }
    private Double HandbookPrice { get; } = 50_0000D;
    private MongoId BaseId { get; } = new("68091a0e9cde8a37ade29400");
    private MongoId NewId { get; } = new("68091a0e9cde8a37ade29401");
    private MongoId RotateId { get; set; } = new("68091a0e9cde8a37ade29420");
...
#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCase545x39 (ISptLogger<AddAmmoCase545x39> logger, DatabaseService databaseService, CustomItemService customItemService) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "5.45x39 ammo case",ShortName = "5.45x39",Description = "skydust™ 5.45x39 ammo case"}},
                {"ch",new(){Name = "5.45x39 弹药箱",ShortName = "5.45x39",Description = "skydust™ 5.45x39 弹药箱"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase545x39.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase545x39.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCase545x39.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
