...
public class AddAmmoCase955x39 : IOnLoad {
    private ISptLogger<AddAmmoCase955x39> Logger { get; }
    private DatabaseService DatabaseService { get; }
    private CustomItemService CustomItemService { get; }
    private Double HandbookPrice { get; } = 50_0000D;
    private MongoId BaseId { get; } = new("68091f7282a7c5319d1a1100");
    private MongoId NewId { get; } = new("68091f7282a7c5319d1a1101");
    private MongoId RotateId { get; set; } = new("68091f7282a7c5319d1a1120");
...
#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCase955x39 (ISptLogger<AddAmmoCase955x39> logger, DatabaseService databaseService, CustomItemService customItemService) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = ".366TKM ammo case",ShortName = ".366TKM",Description = "skydust™ .366TKM ammo case"}},
                {"ch",new(){Name = ".366TKM 弹药箱",ShortName = ".366TKM",Description = "skydust™ .366TKM 弹药箱"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase955x39.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase955x39.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCase955x39.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
