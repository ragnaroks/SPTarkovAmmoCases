...
public class AddAmmoCase9x19Para : IOnLoad {
    private ISptLogger<AddAmmoCase9x19Para> Logger { get; }
    private DatabaseService DatabaseService { get; }
    private CustomItemService CustomItemService { get; }
    private Double HandbookPrice { get; } = 50_0000D;
    private MongoId BaseId { get; } = new("68090533afe569b390391100");
    private MongoId NewId { get; } = new("68090533afe569b390391101");
    private MongoId RotateId { get; set; } = new("68090533afe569b390391120");
...
#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCase9x19Para (ISptLogger<AddAmmoCase9x19Para> logger, DatabaseService databaseService, CustomItemService customItemService) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "9x19Para ammo case",ShortName = "9x19Para",Description = "skydust™ 9x19Para ammo case"}},
                {"ch",new(){Name = "9x19Para 弹药箱",ShortName = "9x19Para",Description = "skydust™ 9x19Para 弹药箱"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase9x19Para.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase9x19Para.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCase9x19Para.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
