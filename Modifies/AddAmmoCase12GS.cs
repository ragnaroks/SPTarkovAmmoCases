...
public class AddAmmoCase12GS : IOnLoad {
    private ISptLogger<AddAmmoCase12GS> Logger { get; }
    private DatabaseService DatabaseService { get; }
    private CustomItemService CustomItemService { get; }
    private Double HandbookPrice { get; } = 50_0000D;
    private MongoId BaseId { get; } = new("68092112e8b8555ab2ecee00");
    private MongoId NewId { get; } = new("68092112e8b8555ab2ecee01");
    private MongoId RotateId { get; set; } = new("68092112e8b8555ab2ecee20");
    private IEnumerable<MongoId> ItemTpls { get; } = [
        ItemTpl.AMMO_12G_AP20,
        ItemTpl.AMMO_12G_50_BMG,
        ItemTpl.AMMO_12G_POLEVA6U,
        ItemTpl.AMMO_12G_FTX,
        ItemTpl.AMMO_12G_POLEVA3,
        ItemTpl.AMMO_12G_DUALSABOT,
        ItemTpl.AMMO_12G_SLUG,
        ItemTpl.AMMO_12G_CSP,
        ItemTpl.AMMO_12G_GRIZZLY_40,
...
#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCase12GS (ISptLogger<AddAmmoCase12GS> logger, DatabaseService databaseService, CustomItemService customItemService) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "12/70 (slug) ammo case",ShortName = "12/70 (slug)",Description = "skydust™ 12/70 (slug) ammo case"}},
                {"ch",new(){Name = "12/70 (slug) 弹药箱",ShortName = "12/70 (slug)",Description = "skydust™ 12/70 (slug) 弹药箱"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase12GS.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase12GS.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCase12GS.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
