...
public class AddAmmoCase23x75 : IOnLoad {
    private ISptLogger<AddAmmoCase23x75> Logger { get; }
    private DatabaseService DatabaseService { get; }
    private CustomItemService CustomItemService { get; }
    private Double HandbookPrice { get; } = 50_0000D;
    private MongoId BaseId { get; } = new("6809266fc1cb707e5ee48100");
    private MongoId NewId { get; } = new("6809266fc1cb707e5ee48101");
    private MongoId RotateId { get; set; } = new("6809266fc1cb707e5ee48120");
    private IEnumerable<MongoId> ItemTpls { get; } = [
        ItemTpl.AMMO_23X75_BARRIKADA,
        ItemTpl.AMMO_23X75_SHRAP10,
        ItemTpl.AMMO_23X75_SHRAP25,
...
#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCase23x75 (ISptLogger<AddAmmoCase23x75> logger, DatabaseService databaseService, CustomItemService customItemService) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "23x75 ammo case",ShortName = "23x75",Description = "skydust™ 23x75 ammo case"}},
                {"ch",new(){Name = "23x75 弹药箱",ShortName = "23x75",Description = "skydust™ 23x75 弹药箱"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase23x75.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase23x75.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
