...
public class AddAmmoCase12GB : IOnLoad {
    private ISptLogger<AddAmmoCase12GB> Logger { get; }
    private DatabaseService DatabaseService { get; }
    private CustomItemService CustomItemService { get; }
    private Double HandbookPrice { get; } = 50_0000D;
    private MongoId BaseId { get; } = new("68092112e8b8555ab2eced00");
    private MongoId NewId { get; } = new("68092112e8b8555ab2eced01");
    private MongoId RotateId { get; set; } = new("68092112e8b8555ab2eced20");
    private IEnumerable<MongoId> ItemTpls { get; } = [
        ItemTpl.AMMO_12G_FLECHETTE,
        ItemTpl.AMMO_12G_PIRANHA,
        ItemTpl.AMMO_12G_EXPRESS,
        ItemTpl.AMMO_12G_7MM,
        ItemTpl.AMMO_12G_MAGNUM,
        ItemTpl.AMMO_12G_RIP,
        ItemTpl.AMMO_12G_525MM,
...
#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCase12GB (ISptLogger<AddAmmoCase12GB> logger, DatabaseService databaseService, CustomItemService customItemService) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "12/70 (buck) ammo case",ShortName = "12/70 (buck)",Description = "skydust™ 12/70 (buck) ammo case"}},
                {"ch",new(){Name = "12/70 (buck) 弹药箱",ShortName = "12/70 (buck)",Description = "skydust™ 12/70 (buck) 弹药箱"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase12GB.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase12GB.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCase12GB.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
