...
public class AddAmmoCase9x21 : IOnLoad {
    private ISptLogger<AddAmmoCase9x21> Logger { get; }
    private DatabaseService DatabaseService { get; }
    private CustomItemService CustomItemService { get; }
    private Double HandbookPrice { get; } = 50_0000D;
    private MongoId BaseId { get; } = new("68090b87e3db563375934000");
    private MongoId NewId { get; } = new("68090b87e3db563375934001");
    private MongoId RotateId { get; set; } = new("68090b87e3db563375934020");
...
#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCase9x21 (ISptLogger<AddAmmoCase9x21> logger, DatabaseService databaseService, CustomItemService customItemService) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "9x21 ammo case",ShortName = "9x21",Description = "skydust™ 9x21 ammo case"}},
                {"ch",new(){Name = "9x21 弹药箱",ShortName = "9x21",Description = "skydust™ 9x21 弹药箱"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase9x21.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase9x21.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCase9x21.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
