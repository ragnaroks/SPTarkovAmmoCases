...
public class AddAmmoCase68x51 : IOnLoad {
    private ISptLogger<AddAmmoCase68x51> Logger { get; }
    private DatabaseService DatabaseService { get; }
    private CustomItemService CustomItemService { get; }
    private Double HandbookPrice { get; } = 50_0000D;
    private MongoId BaseId { get; } = new("68091b00402733a5848f8d00");
    private MongoId NewId { get; } = new("68091b00402733a5848f8d01");
    private MongoId RotateId { get; set; } = new("68091b00402733a5848f8d20");
...
#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCase68x51 (ISptLogger<AddAmmoCase68x51> logger, DatabaseService databaseService, CustomItemService customItemService) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "6.8x51 ammo case",ShortName = "6.8x51",Description = "skydust™ 6.8x51 ammo case"}},
                {"ch",new(){Name = "6.8x51 弹药箱",ShortName = "6.8x51",Description = "skydust™ 6.8x51 弹药箱"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase68x51.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase68x51.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCase68x51.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
