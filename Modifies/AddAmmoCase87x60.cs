...
public class AddAmmoCase87x60 : IOnLoad {
    private ISptLogger<AddAmmoCase87x60> Logger { get; }
    private DatabaseService DatabaseService { get; }
    private CustomItemService CustomItemService { get; }
    private Double HandbookPrice { get; } = 50_0000D;
    private MongoId BaseId { get; } = new("68091e58beb8727e1dcd2200");
    private MongoId NewId { get; } = new("68091e58beb8727e1dcd2201");
    private MongoId RotateId { get; set; } = new("68091e58beb8727e1dcd2220");
...
#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCase87x60 (ISptLogger<AddAmmoCase87x60> logger, DatabaseService databaseService, CustomItemService customItemService) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = ".338 Lapua Magnum ammo case",ShortName = ".338 Lapua Magnum",Description = "skydust™ .338 Lapua Magnum ammo case"}},
                {"ch",new(){Name = ".338 Lapua Magnum 弹药箱",ShortName = ".338 Lapua Magnum",Description = "skydust™ .338 Lapua Magnum 弹药箱"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase87x60.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase87x60.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCase87x60.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
