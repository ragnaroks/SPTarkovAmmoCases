...
public class AddAmmoCase20GB : IOnLoad {
    private ISptLogger<AddAmmoCase20GB> Logger { get; }
    private DatabaseService DatabaseService { get; }
    private CustomItemService CustomItemService { get; }
    private Double HandbookPrice { get; } = 50_0000D;
    private MongoId BaseId { get; } = new("68092504f821917d87510100");
    private MongoId NewId { get; } = new("68092504f821917d87510101");
    private MongoId RotateId { get; set; } = new("68092504f821917d87510120");
    private IEnumerable<MongoId> ItemTpls { get; } = [
        ItemTpl.AMMO_20G_FLECHETTE,
        ItemTpl.AMMO_20G_75MM,
        ItemTpl.AMMO_20G_73MM,
        ItemTpl.AMMO_20G_62MM,
        ItemTpl.AMMO_20G_56MM,
...
#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCase20GB (ISptLogger<AddAmmoCase20GB> logger, DatabaseService databaseService, CustomItemService customItemService) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "20/70 (buck) ammo case",ShortName = "20/70 (buck)",Description = "skydust™ 20/70 (buck) ammo case"}},
                {"ch",new(){Name = "20/70 (buck) 弹药箱",ShortName = "20/70 (buck)",Description = "skydust™ 20/70 (buck) 弹药箱"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase20GB.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase20GB.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCase20GB.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
