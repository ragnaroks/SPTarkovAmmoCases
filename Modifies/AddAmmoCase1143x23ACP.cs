...
public class AddAmmoCase1143x23ACP : IOnLoad {
    private ISptLogger<AddAmmoCase1143x23ACP> Logger { get; }
    private DatabaseService DatabaseService { get; }
    private CustomItemService CustomItemService { get; }
    private Double HandbookPrice { get; } = 50_0000D;
    private MongoId BaseId { get; } = new("680914f9931484e2a14f9800");
    private MongoId NewId { get; } = new("680914f9931484e2a14f9801");
    private MongoId RotateId { get; set; } = new("680914f9931484e2a14f9820");
...
#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCase1143x23ACP (ISptLogger<AddAmmoCase1143x23ACP> logger, DatabaseService databaseService, CustomItemService customItemService) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = ".45ACP ammo case",ShortName = ".45ACP",Description = "skydust™ .45ACP ammo case"}},
                {"ch",new(){Name = ".45ACP 弹药箱",ShortName = ".45ACP",Description = "skydust™ .45ACP 弹药箱"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase1143x23ACP.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase1143x23ACP.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCase1143x23ACP.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
