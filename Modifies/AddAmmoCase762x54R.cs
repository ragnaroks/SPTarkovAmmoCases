...
public class AddAmmoCase762x54R : IOnLoad {
    private ISptLogger<AddAmmoCase762x54R> Logger { get; }
    private DatabaseService DatabaseService { get; }
    private CustomItemService CustomItemService { get; }
    private Double HandbookPrice { get; } = 50_0000D;
    private MongoId BaseId { get; } = new("68091e451ae2dca245543b00");
    private MongoId NewId { get; } = new("68091e451ae2dca245543b01");
    private MongoId RotateId { get; set; } = new("68091e451ae2dca245543b20");
...
#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCase762x54R (ISptLogger<AddAmmoCase762x54R> logger, DatabaseService databaseService, CustomItemService customItemService) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "7.62x54R ammo case",ShortName = "7.62x54R",Description = "skydust™ 7.62x54R ammo case"}},
                {"ch",new(){Name = "7.62x54R 弹药箱",ShortName = "7.62x54R",Description = "skydust™ 7.62x54R 弹药箱"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase762x54R.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase762x54R.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCase762x54R.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
