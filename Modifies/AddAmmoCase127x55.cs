...
public class AddAmmoCase127x55 : IOnLoad {
    private ISptLogger<AddAmmoCase127x55> Logger { get; }
    private DatabaseService DatabaseService { get; }
    private CustomItemService CustomItemService { get; }
    private Double HandbookPrice { get; } = 50_0000D;
    private MongoId BaseId { get; } = new("68091fff09a0386549a3f500");
    private MongoId NewId { get; } = new("68091fff09a0386549a3f501");
    private MongoId RotateId { get; set; } = new("68091fff09a0386549a3f520");
...
#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCase127x55 (ISptLogger<AddAmmoCase127x55> logger, DatabaseService databaseService, CustomItemService customItemService) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "12.7x55 ammo case",ShortName = "12.7x55",Description = "skydust™ 12.7x55 ammo case"}},
                {"ch",new(){Name = "12.7x55 弹药箱",ShortName = "12.7x55",Description = "skydust™ 12.7x55 弹药箱"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase127x55.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase127x55.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCase127x55.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
