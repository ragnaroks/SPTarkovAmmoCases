...
public class AddAmmoCase127x33AE : IOnLoad {
    private ISptLogger<AddAmmoCase127x33AE> Logger { get; }
    private DatabaseService DatabaseService { get; }
    private CustomItemService CustomItemService { get; }
    private Double HandbookPrice { get; } = 50_0000D;
    private MongoId BaseId { get; } = new("680915e32e4f2e1ef0b88900");
    private MongoId NewId { get; } = new("680915e32e4f2e1ef0b88901");
    private MongoId RotateId { get; set; } = new("680915e32e4f2e1ef0b88920");
...
#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCase127x33AE (ISptLogger<AddAmmoCase127x33AE> logger, DatabaseService databaseService, CustomItemService customItemService) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = ".50AE ammo case",ShortName = ".50AE",Description = "skydust™ .50AE ammo case"}},
                {"ch",new(){Name = ".50AE 弹药箱",ShortName = ".50AE",Description = "skydust™ .50AE 弹药箱"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase127x33AE.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase127x33AE.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCase127x33AE.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
