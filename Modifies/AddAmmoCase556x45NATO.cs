...
public class AddAmmoCase556x45NATO : IOnLoad {
    private ISptLogger<AddAmmoCase556x45NATO> Logger { get; }
    private DatabaseService DatabaseService { get; }
    private CustomItemService CustomItemService { get; }
    private Double HandbookPrice { get; } = 50_0000D;
    private MongoId BaseId { get; } = new("68091a1867d7cee59666b800");
    private MongoId NewId { get; } = new("68091a1867d7cee59666b801");
    private MongoId RotateId { get; set; } = new("68091a1867d7cee59666b820");
...
#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCase556x45NATO (ISptLogger<AddAmmoCase556x45NATO> logger, DatabaseService databaseService, CustomItemService customItemService) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "5.56x45NATO ammo case",ShortName = "5.56x45NATO",Description = "skydust™ 5.56x45NATO ammo case"}},
                {"ch",new(){Name = "5.56x45NATO 弹药箱",ShortName = "5.56x45NATO",Description = "skydust™ 5.56x45NATO 弹药箱"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase556x45NATO.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase556x45NATO.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCase556x45NATO.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
