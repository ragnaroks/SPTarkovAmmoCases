...
public class AddAmmoCasePL2 : IOnLoad {
    private ISptLogger<AddAmmoCasePL2> Logger { get; }
    ...
    private MongoId BaseId { get; } = new("68108b05b9a977d3588d1d00");
    private MongoId NewId { get; } = new("68108b05b9a977d3588d1d01");
    private MongoId RotateId { get; set; } = new("68108b05b9a977d3588d1d20");

#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCasePL2 (ISptLogger<AddAmmoCasePL2> logger, DatabaseService databaseService, CustomItemService customItemService, ItemHelper itemHelper) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        foreach (MongoId id in ammoTpls) {
            ...
            if (template.Properties.PenetrationPower is null or < 20 or > 29) { continue; }
            _ = itemTpls.Add(id);
        }
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "PL2 ammo case",ShortName = "PL2",Description = "skydust™ PL2 ammo case, can store ammos those penetration power between 20 and 29"}},
                {"ch",new(){Name = "PL2 弹药箱",ShortName = "PL2",Description = "skydust™ PL2 弹药箱，可存放穿透力 20 至 29 的弹药"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCasePL2.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCasePL2.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCasePL2.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
