...
public class AddAmmoCase20GS : IOnLoad {
    private ISptLogger<AddAmmoCase20GS> Logger { get; }
    private DatabaseService DatabaseService { get; }
    private CustomItemService CustomItemService { get; }
    private Double HandbookPrice { get; } = 50_0000D;
    private MongoId BaseId { get; } = new("680925c2d27153a12cb4a800");
    private MongoId NewId { get; } = new("680925c2d27153a12cb4a801");
    private MongoId RotateId { get; set; } = new("680925c2d27153a12cb4a820");
    private IEnumerable<MongoId> ItemTpls { get; } = [
        ItemTpl.AMMO_20G_TSS_AP,
        ItemTpl.AMMO_20G_DGS,
        ItemTpl.AMMO_20G_POLEVA6U,
        ItemTpl.AMMO_20G_STAR,
        ItemTpl.AMMO_20G_POLEVA3,
...
#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCase20GS (ISptLogger<AddAmmoCase20GS> logger, DatabaseService databaseService, CustomItemService customItemService) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "20/70 (slug) ammo case",ShortName = "20/70 (slug)",Description = "skydust™ 20/70 (slug) ammo case"}},
                {"ch",new(){Name = "20/70 (slug) 弹药箱",ShortName = "20/70 (slug)",Description = "skydust™ 20/70 (slug) 弹药箱"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase20GS.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase20GS.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCase20GS.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
