...
public class AddAmmoCase762x25TT : IOnLoad {
    private ISptLogger<AddAmmoCase762x25TT> Logger { get; }
    private DatabaseService DatabaseService { get; }
    private CustomItemService CustomItemService { get; }
    private Double HandbookPrice { get; } = 50_0000D;
    private MongoId BaseId { get; } = new("68090df530a3d650ee091400");
    private MongoId NewId { get; } = new("68090df530a3d650ee091401");
    private MongoId RotateId { get; set; } = new("68090df530a3d650ee091420");
...
#pragma warning disable IDE0290 // 使用主构造函数
    public AddAmmoCase762x25TT (ISptLogger<AddAmmoCase762x25TT> logger, DatabaseService databaseService, CustomItemService customItemService) {
        this.Logger = logger;
...
#pragma warning restore IDE0290 // 使用主构造函数
    ...
    public Task OnLoad () {
        ...
        NewItemFromCloneDetails newItem = new() {
            ...
            Locales = new(){
                {"en",new(){Name = "7.62x25TT ammo case",ShortName = "7.62x25TT",Description = "skydust™ 7.62x25TT ammo case"}},
                {"ch",new(){Name = "7.62x25TT 弹药箱",ShortName = "7.62x25TT",Description = "skydust™ 7.62x25TT 弹药箱"}}
            },
        ...
        if (createItemResult.Success is false) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase762x25TT.OnLoad() / failed / ", String.Join("；", createItemResult.Errors ?? Enumerable.Empty<String>())),
                LogTextColor.Yellow
            );
        ...
        if (trader is null) {
            this.Logger.Log(
                LogLevel.Info,
                String.Concat(Constants.LoggerPrefix, "AddAmmoCase762x25TT.OnLoad() / failed / trader not found"),
                LogTextColor.Yellow
            );
            return Task.CompletedTask;
        }
        ...
        this.Logger.Log(
            LogLevel.Info,
            String.Concat(Constants.LoggerPrefix, "AddAmmoCase762x25TT.OnLoad() / success / ", this.BaseId, " / ", this.RotateId),
            LogTextColor.Green
        );
        return Task.CompletedTask;
    }
